            ...
            <div className="ml-10 hidden md:flex items-center space-x-4">
              ...
              {isAuthenticated && user?.isAdmin && (
                <div>
                  <Link 
                    href="/admin" 
                    className={`px-3 py-2 rounded-md text-sm font-medium hover:bg-blue-700 duration-300 ${isActive('/admin')} mr-4`}
                  >
                    Admin Dashboard
                  </Link>
                  {/* <Link 
          ...
          <div className="flex flex-col items-start pb-2 gap-1">
            ...
            {isAuthenticated && user?.isAdmin && (
              <>
                <Link 
                  href="/admin" 
                  className={`py-4 px-2 w-full rounded-md text-sm font-medium hover:bg-blue-700 duration-300 ${isActive('/admin')}`}
                  onClick={toggleMenu}
                >
                  Admin Dashboard
                </Link>
                <Link 
                  href="/admin/create" 
                  className={`py-4 px-2 w-full rounded-md text-sm font-medium hover:bg-blue-700 duration-300 ${isActive('/admin/create')}`}
                  onClick={toggleMenu}
                >
                  Create Tale
                </Link>
